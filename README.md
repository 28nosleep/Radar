...
Персональная разведывательная лента для одного владельца. Текущий Milestone M2 —
полностью проходящий вертикальный срез:

`RSS / Hacker News / arXiv → нормализация → классификация → дедупликация → rule-based TOP-10 → OpenAI → личный Telegram`
...
2. **M2 — Hacker News и arXiv (текущий).** Два бесплатных коллектора подключаются к
