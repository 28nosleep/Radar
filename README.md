...
Персональная разведывательная лента для одного владельца. Текущий Milestone M1 —
полностью проходящий вертикальный срез:

`RSS → нормализация → классификация → дедупликация → rule-based TOP-10 → OpenAI → личный Telegram`
...
1. **M1 — RSS vertical slice (текущий).** Фиксированный JSON-каталог RSS, общая
   модель, консервативные дубли, keyword-категории, объяснимый score, максимум TOP-10
   запросов к OpenAI и Telegram/dry-run.
2. **M2 — Hacker News и arXiv.** Два бесплатных коллектора подключаются к той же
   модели; весь downstream-пайплайн остаётся неизменным.
3. **M3 — GitHub, Reddit и YouTube.** Добавляются официальные API и снимки метрик.
   X подключается только при наличии приемлемого бесплатного официального доступа;
   хрупкий скрейпинг не является обязательным путём.
...
FastAPI и отдельные сервисы для M1 не нужны. Запуски не пересекаются; RSS внутри
...
- `adapters/` — RSS, OpenAI Responses API и Telegram Bot API;
...
название, URL ленты, репутация `0..1`, флаг активности и категории по умолчанию.
Удалённый из каталога источник выключается в базе, но его история сохраняется.

Коллектор поддерживает RSS/Atom, ETag/Last-Modified, таймаут и ограничение размера.
Ошибка одной ленты не останавливает остальные.
